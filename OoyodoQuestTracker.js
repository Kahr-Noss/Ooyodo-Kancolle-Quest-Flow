$(function () {
  ...
  function initialisation(){
    ...
    $(document).bind("bubble_displayed",function(){
          ...
          */
          ...
          if(inconsistenciesList.length !== 0){
            ...
            askForUnknowQuestState(unknownQuestsGroups,userQuestCookie.userDecisions, userQuestCookie.undeterminedQuests, function(userDecisions, undeterminedQuests){
              ...
              setPeriodicQuestState(userQuestCookie.pendingQuests,userQuestCookie.periodicCompleted);

              // update the ALL_QUEST STATE list with the one just calculated if on inconsistencies
              questStateCalculated = true;
              implementQuestsStateUpdated(userQuestCookie.pendingQuests,userDecisions, undeterminedQuests, userQuestCookie.periodicCompleted);
            });
          }
        }
      }
      ...
      function askForUnknowQuestState(unknowQuestsGroup, userDecisions, undeterminedQuests, callback){
        ...
        if (unknowQuestsGroup.length > 0){
          ...
          if (ALL_QUESTS_LIST[questsGroup[0]].requires.every(function(requiredQuest){return ALL_QUEST_STATE_TMP[requiredQuest] === "completed"})){
            ...
            //TODO add a link to display again the quests, and replace the those
            displayBubbleMessage(`Admiral, I need some help to<br>
              ...
              <span class="link" id="MSG_ask_quest_state_display">${questsGroup.join(", ")}</span><br>
      ...
      function implementQuestsStateUpdated(pendingQuests, userDecisions, undeterminedQuests, periodicCompleted){
        ...
        setCookie('user_quests',JSON.stringify({pendingQuests:pendingQuests, userDecisions:userDecisions, periodicCompleted:periodicCompleted, undeterminedQuests:undeterminedQuests, timeStamp:moment().utcOffset("+09:00").format()}),365);
        ...
        getQuestsInState(ALL_QUEST_STATE_TMP,'???').forEach(quest=>{
          if(periodNumberEquvalence(ALL_QUESTS_LIST[quest].period) === 5){
            if (ALL_QUESTS_LIST[quest].requires.every(function(requiredQuest){return ALL_QUEST_STATE_TMP[requiredQuest] === "completed";})){
              ...
              if (searchPendingOrCompletedQuestInUnlocks(previousQuest,"once") &&  periodNumberEquvalence(ALL_QUESTS_LIST[previousQuest].period) <= 4){
                ALL_QUEST_STATE_TMP[quest] = "completed";
                ALL_QUESTS_LIST[quest].unlocks.forEach(nextQuest => {
                  setUnknownQuestsUpward(nextQuest,"completed","once");
                });
                undeterminedQuests[quest] = "completed";
              } else {
      ...
      function setPeriodicQuestState(pendingQuests, periodicCompleted){
        // do it in decreasing period time order
        ["daily","weekly","monthly","quarterly"].forEach(period => {
          ...
          getQuestsInState(ALL_QUEST_STATE_TMP,'???').filter(function(qst){return ALL_QUESTS_LIST[qst].period === period}).forEach(quest=>{
            ...
            if (ALL_QUESTS_LIST[quest].requires.every(function(requiredQuest){return ALL_QUEST_STATE_TMP[requiredQuest] === "completed";})){
              ...
              setUnknownQuestsUpward(quest,"completed",period);
              ...
              setUnknownQuestsUpward(quest,"locked",period);

            } else if( $.inArray(period,periodicCompleted) !== -1 ){
              ALL_QUEST_STATE_TMP[quest] = "completed";
              ALL_QUESTS_LIST[quest].unlocks.forEach(nextQuest => {
                setUnknownQuestsUpward(nextQuest,"completed",period);
              });
              ...
              var previousQuest = ALL_QUESTS_LIST[quest].requires[0];
              if (searchPendingOrCompletedQuestInUnlocks(previousQuest,period) &&  periodNumberEquvalence(ALL_QUESTS_LIST[previousQuest].period) <= periodNumberEquvalence(period)){
                ALL_QUEST_STATE_TMP[quest] = "completed";
                ALL_QUESTS_LIST[quest].unlocks.forEach(nextQuest => {
                  setUnknownQuestsUpward(nextQuest,"completed",period);
                });
      ...
      function setUnknownQuestsUpward(quest,state,period){
      ...
      function searchPendingOrCompletedQuestInUnlocks(quest,period){
        ...
        ALL_QUESTS_LIST[quest].unlocks.forEach(unlockedQuest =>{
          ...
          && periodNumberEquvalence(ALL_QUESTS_LIST[unlockedQuest].period) <= periodNumberEquvalence(period)){
            isFound = true;
          } else {
